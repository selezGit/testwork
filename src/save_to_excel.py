...
def save_to_excel(data, offset, exchange):
    ...
    ws.cell(column=offset+1,
            row=1,
            value='Дата',
            ).style = config.HEADER_STYLE
    ws.cell(column=offset+2,
            row=1,
            value='Курс',
            ).style = config.HEADER_STYLE    
        
    ws.cell(column=offset+3,
            row=1,
            value='Изменение',
            ).style = config.HEADER_STYLE
    ...
    for row, date in enumerate(data.keys()):
        ...
        cell_date = ws.cell(column=offset+1, row=row+2,
                value=date).style = config.BASE_STYLE
        cell_date.number_format = currency
        
        cell_rate = ws.cell(column=offset+2,
                            row=row+2,
                            value=exchange_rate,
